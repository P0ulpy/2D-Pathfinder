...
double AStar::manhattan(sf::source, sf::Vector2i target)
{
    const auto delta = std::move(getDelta(source, target));
    return 10 * (delta.x + delta.y);
}

static sf::Vector2i AStar::getDelta(sf::Vector2i source_, sf::Vector2i target_)
{
    return{ abs(source_.x - target_.x),  abs(source_.y - target_.y) };
}

const std::list<SharedPtrNodeTile2D> AStar::RunAStar(const SharedPtrNodeTile2D& startingNode, const SharedPtrNodeTile2D& goalNode)
{
